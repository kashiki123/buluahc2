...
$sql = "SELECT *,prenatal_consultation.id as id,midwife.last_name as last_name
FROM prenatal_consultation
JOIN patients ON prenatal_consultation.patient_id = patients.id
                ...
                <div class="modal-body">
                    <form id="editForm">
                            ...
                            <select class="form-control" name="doctor_id" id="editDoctor" required>
                                ...
                                if ($result2->num_rows > 0) {
                                    while ($row2 = $result2->fetch_assoc()) {
                                        ...
                                        // Output an option element for each doctor
                                        echo "<option value='$doctorId'>$firstName $lastName</option>";
                                    }
                                } else {
                                    echo "<option disabled>No Doctor Found</option>";
                                }
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="editPrescription" name="medicine" rows="3"
                                required></textarea>
                        </div>
                    </form>
                </div>
...
<script>
    $(document).ready(function () {
                ...
                columnDefs: [
                    ...
                    { targets: 5, data: 'medicine' },
                    {
                        ...
                        render: function (data, type, row) {
                            var editButton = '<button type="button" class="btn btn-info editmodal" data-row-id="' + row.id + '"><i class="fas fa-eye"></i> View</button>';
        ...
        $('#tablebod').on('click', '.editmodal', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#editModal #editPrescription').val(editGetData.medicine);
                    $('#editModal #editDoctor').val(editGetData.midwife_id);
