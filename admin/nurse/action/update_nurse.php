...
try {
    ...
    $check_email_sql = "SELECT COUNT(*) FROM users WHERE email = ? AND id != (SELECT user_id FROM nurses WHERE id = ?)";
    ...
    if (isset($password) && $password !== '') {
        ...
        $usersUpdateSql = "UPDATE users SET password=?, email=? WHERE id=(SELECT user_id FROM nurses WHERE id=?)";
        ...
        $usersUpdateSql = "UPDATE users SET email=? WHERE id=(SELECT user_id FROM nurses WHERE id=?)";
        $usersStmt = $conn->prepare($usersUpdateSql);
        if (!$usersStmt) {
            throw new Exception("Failed to prepare statement: " . $conn->error);
        }
