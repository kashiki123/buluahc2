...
try {
    ...
    $consultationUpdateSql = "UPDATE consultations SET steps=?, status=?, subjective=?, objective=?, assessment=?, plan=?, checkup_date=?, doctor_id=? WHERE id=?";
    $consultationStmt = $conn->prepare($consultationUpdateSql);
    $consultationStmt->bind_param("ssssssssi", $steps, $status, $subjective, $objective, $assessment, $plan, $checkup_date, $doctor_id, $primary_id);
