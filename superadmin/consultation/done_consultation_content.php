...
WHERE consultations.is_active = 0 AND superadmins.user_id = $user_id AND consultations.is_print = 1";
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        <!-- <button type="button" class="btn btn-success editbtn" data-row-id="<?php echo $row['serial_no']; ?>">
                                    <i class="fas fa-eye"></i> View Consultation
                                        ...
                                        <form method="POST" action="generate-referral.php?id=<?php echo $row['serial_no']; ?>">
                                            <button type="submit" class="btn  btn-info mt-2 referralbtn"><i
                                                    class="fas fa-file-alt"></i> Generate Referral</button>
                                        </form>
                ...
                columnDefs: [
                    ...
                    { targets: 3, data: 'checkup_date' },
                    {
                        ...
                        render: function (data, type, row) {
                            var editButton = '<button type="button" class="btn btn-success editbtn" data-row-id="' + row.serial_no + '"><i class="fas fa-eye"></i>  View Consultation</button>';
                            ...
                            var receiptButton = '<form method="POST" action="generate-receipt.php?id=' + row.serial_no + '"><button type="submit" class="btn btn-primary mt-2 printbtn"><i class="fa fa-file"></i> Generate Prescription</button></form>';
                            var referralButton = '<form method="POST" action="generate-referral.php?id= ' + row.serial_no + '"><button type="submit" class="btn  btn-info mt-2 referralbtn"><i class="fas fa-file"></i> Generate Referral</button></form>';

                            return receiptButton + ' ' + referralButton;
                        }
                    } // Action column
                ],
